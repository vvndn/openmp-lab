...
- loop unrolled part of OMP_Deblur, added some #pragma omp parallel for num_threads(16) private (x, y) for the section with no data dependencies to achieve more speedup
- code motion/common subexpressions/strength reductions
...
Test runs with this code yielded the following times (on server 08, around 7pm June 8th):
...
void OMP_GaussianBlur(double *u, double Ksigma, int stepCount)
{
	...
	{
		// beginning of dead code elimination
		/*
		...
		*/
		for(z = 1; z < zMax; z++)
		{
			for(y = 0; y < yMax; y++)
			{
				for(x = 0; x < xMax; x += 16)
				{
					// use xyzInd to avoid recalculation of Index every time
					// loop unrolling by 16 per iteration
					int xyzInd = Index(x, y, z);
...
void OMP_Deblur(double* u, const double* f, int maxIterations, double dt, double gamma, double sigma, double Ksigma)
{
	...
	{
		// parallel here since there are no data dependencies
		#pragma omp parallel for num_threads(16) private (x, y)
		for(z = 1; z < zMax - 1; z++)
		{
			for(y = 1; y < yMax - 1; y++)
			{
				for(x = 1; x < xMax - 1; x += 2)
				{
					...
					// loop unroll by 2 (any more caused errors)
		...
		// parallel here since there are also no data dependencies here
		#pragma omp parallel for num_threads(16) private (x, y)
		for(z = 0; z < zMax; z++)
		{
			for(y = 0; y < yMax; y++)
