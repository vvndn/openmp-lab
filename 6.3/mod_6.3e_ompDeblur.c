		...
		for(z = 1; z < zMax - 1; z++)
		{
			for(x = 1; x < xMax - 1; x++)
			{
				for(y = 1; y < yMax - 1; y++)
				{
					// more code motion
					int xyzInd = Index(x, y, z);
					int xyz_xmin = Index(x - 1, y, z);
					int xyz_xplu = Index(x + 1, y, z);
					...
					double newVal = (u[xyzInd] + dt * ( 
						u[xyz_xmin] * g[xyz_xmin] + 
						u[xyz_xplu] * g[xyz_xplu] + 
						u[xyz_ymin] * g[xyz_ymin] + 
						u[xyz_yplu] * g[xyz_yplu] + 
						u[xyz_zmin] * g[xyz_zmin] + 
						u[xyz_zplu] * g[xyz_zplu] - gamma * conv[xyzInd])) /
						(1.0 + dt * (g[xyz_xplu] + g[xyz_xmin] + g[xyz_yplu] + g[xyz_ymin] + g[xyz_zplu] + g[xyz_zmin]));
