...
void OMP_Initialize(int xM, int yM, int zM)
{
	...
	yMax = yM;
	zMax = zM;
	assert(OMP_conv = (double*)malloc(sizeof(double) * 2097152));
	assert(OMP_g = (double*)malloc(sizeof(double) * 2097152));
}
void OMP_Finish()
{
	free(OMP_conv);
